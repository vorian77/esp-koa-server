...
const getApplicantData = async function (ctx) {
  ...
  try {
    ...
    ctx.state.site_current = response.data.site_current
    ctx.state.site_previous = response.data.site_previous
  } catch (err) {
    console.error(
      `getApplicantData error...\nURL: ${url} \nQuery Params: ${JSON.stringify(
        ctx.query,
      )}`,
    )
...
const setEmailContent = async function (ctx) {
  ...
  const siteCurrent = ctx.state.site_current
  const sitePrevious = ctx.state.site_previous

  // email subject
  ctx.state.emailSubject =
    "KidSmart - New Self-Service App File Update - " +
    alertType +
    " (" +
    applicantName +
    ")"
  ...
  if (content) {
    emailBody += "<p></p><p><b>Message Content: </b>" + content + "</p>"
  }
